...
<form id="forms" class="form-horizontal" method="post">
	<div class="form-group"> 
		...
		<div class="col-sm-7"> 
			<input type="text" class="form-control validate[required,custom[onlyNnumbers],custom[integer]]" name="number" id="number" /> 
		</div> 
	</div>
	<div class="form-group"> 
		<label class="col-sm-5 control-label"><?=$k101?>:</label> 
		<div class="col-sm-7"> 
			<input type="text" class="form-control validate[required]" name="inv" id="inv" /> 
		</div> 
	</div>
	<div class="form-group"> 
	...
	<div class="form-group"> 
		...
		<div class="col-sm-7"> 
			<input type="text" class="form-control validate[required]" name="brand" id="brand" /> 
		</div> 
	</div>
	<div class="form-group"> 
		<label class="col-sm-5 control-label"><?=$k107?>:</label> 
		<div class="col-sm-7"> 
			<input type="text" class="form-control validate[required]" name="model" id="model" /> 
		</div> 
	</div>
	<div class="form-group"> 
		<label class="col-sm-5 control-label"><?=$wa05?> (<?=$wa07?>):</label> 
		<div class="col-sm-7"> 
			<input type="text" class="form-control validate[required,custom[onlyNnumbers],custom[integer]]" name="kg" id="kg" /> 
		</div> 
	</div>
	<div class="form-group"> 
		<label class="col-sm-5 control-label"><?=$k118?>:</label> 
		<div class="col-sm-7"> 
			<input type="text" class="form-control validate[required,custom[onlyNnumbers],custom[integer]]" name="water_before" id="water_before" /> 
		</div> 
	</div>
	<div class="form-group"> 
		<label class="col-sm-5 control-label"><?=$k119?>:</label> 
		<div class="col-sm-7"> 
			<input type="text" class="form-control validate[required,custom[onlyNnumbers],custom[integer]]" name="water_after" id="water_after" /> 
		</div> 
	</div>
	<div class="form-group"> 
...
<script type="text/javascript">
    ...
    jQuery("#forms").on('click', '#submit', function(e) {
      ...
      if (valid == true) {
        ...
        var datastr = 'number='+number+'&inv='+inv+'&name='+name+'&brand='+brand+'&model='+model+'&kg='+kg+'&water_before='+water_before+'&water_after='+water_after+'&place='+place;
        //console.log(datastr);
        send(datastr);
      } else {
      	e.preventDefault();
    ...
    function send(data) {
        jQuery.ajax({
            type: "POST",
            url: "ajax/settings_washing_machines_add.php",
            data: data,
